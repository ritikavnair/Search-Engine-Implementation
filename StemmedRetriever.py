...
def output_to_file(doc_scores, query_id):
    ...
    output_file = os.path.join(RUN_OUTPUTS_DIR,"Stemmed"+RETRIEVAL_MODEL+"Run.txt")
    rank = 0
    ...
    with io.open(output_file ,"a+") as textfile:
        ...
        for i in range(min(len(sorted_scores),100)):
            k,v = sorted_scores[i]
            rank += 1
            textfile.write(str(query_id) + " " + "Q0 "+ k + " " + str(rank) + " " + str(v) +" Stemmed"+RETRIEVAL_MODEL+"Model\n") 
...
def extract_queries_from_file():
    ...
    extracted_queries = open("cacm_stem.query.txt",'r').read()
    queries = extracted_queries.split('\n')    
    ...
    for query_term in query_term_freq:
        ...
        for doc in fetched_index[query_term]:   
            ...
            if doc in DOC_SCORE_QLM:
                DOC_SCORE_QLM[doc] += float(math.log(first_part + second_part))
            else:
                DOC_SCORE_QLM[doc] = float(math.log(first_part + second_part))
...
def main():
    ...
    INVERTED_INDEX = StemmedIndexer.INVERTED_INDEX
    global DOC_TOKEN_COUNT
    DOC_TOKEN_COUNT = StemmedIndexer.DOC_TOKEN_COUNT
